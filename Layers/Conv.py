...
#########################################################################################################
        ...
        #             tmp += sgl.convolve2d(self.input_tensor[b, c, :, :], np.rot90(self.weights[h, c, :, :],2), "same")
    ...
    def backward(self, error_tensor):
        ...
        for b in range(self.B):  # in each batch_element
            for h in range(self.H):  # in each convolutional kernel
                ...
                E_h[0: self.Y: self.stride[0], 0: self.X: self.stride[1]] = error_tensor[b, h, :, :]
                ...
                for c in range(self.C):  # in each channel
                    ...
                    self.grad_weights[h, c, :, :] += sgl.convolve2d(a_c_padding, np.rot90(E_h,2), "valid")
